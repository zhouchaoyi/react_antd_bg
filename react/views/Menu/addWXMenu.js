...
import {addMenu} from '../../actions/weChat_menu'
...
class AddSubMenu extends React.Component{
    ...
    handleSubmit(e){
        ...
        this.props.form.validateFields((errors,value)=>{
            if(!!errors){
                ...
                this.props.addMenu(this.props.form.getFieldsValue())
                this.back();
    ...
    render(){
        ...
        const nameProps = getFieldProps('name', {
            rules: [
                { required: true, min: 1,max:4, message: '字数不超过4个汉字或8个字母' }
            ],
            onChange: (e) => {
        ...
        return(
            <div>
                <h1 style={{marginBottom:'10px'}}>添加微信一级菜单</h1>
                <Form horizontal onSubmit={this.handleSubmit.bind(this)}>
                    <FormItem
                        ...
                        >
                        <Input type="text" {...nameProps} placeholder="一级菜单名称"  />
                        <p>字数不超过4个汉字或8个字母</p>
                    </FormItem>
                    <FormItem
                        {...formItemLayout}
...
function mapDispatchToProps(dispatch){
    return{
        addMenu:bindActionCreators(addMenu,dispatch)
    }
}
AddSubMenu.contextTypes={
    router:PropTypes.object.isRequired
}
AddSubMenu=Form.create()(AddSubMenu)
export default connect(mapStateToProps,mapDispatchToProps)(AddSubMenu)